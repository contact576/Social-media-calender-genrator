...
6) AUDIO-TREND BOOSTER (optional, budget-gated — only if under the item cap): for the top 2–3 confirmed
   winners, take each reel's `musicInfo.audio_id` and build its audio-page URL, then MCP —
   kinaesthetic_millionaire/instagram-reels-audio-scraper:
   { "audioUrls": ["https://www.instagram.com/reels/audio/<audio_id>/"], "maxResults": 20 }
   (the actor takes an audioUrls ARRAY of audio-page URLs, NOT a bare audio_id). A sound whose last-7-days
   reel count clearly exceeds the prior 7–30 days is STILL trending (jump-worthy NOW) vs "was trending".
   Tag such reels "TRENDING-AUDIO". This is enrichment — it never blocks the run and does not count toward
   the freshness/market-mix guarantees.
...
VAULT SAVE: save TWO files —
  "{{VAULT_FOLDER}}/s4-decode.md" — full cards + HANDOFF SUMMARY (≤30 lines: card IDs with one-line hook
    + format + outlier score + ORIGIN each; mark any UNAVAILABLE/low-confidence cards). Each card RECORDS
    the decode-actor runId + datasetId that produced its VERBAL (and VISUAL if run) so Gate-3 can re-fetch
    and diff them — a card with no recorded actor run cannot be spot-checked and is treated as UNVERIFIED;
...
- MIX every week toward the 40/40/20 split:
  ...
  client" rather than inventing a result. ALWAYS include ≥1 CONVERT slot (it is the revenue bucket) —
  a 0-CONVERT calendar is REJECTED even when no proof is on file (stamp it "needs proof").
- Honor the cadence in {{POSTING_CAPACITY}}, up to a CAP of 7 reels/week; a week with more slots than
  the stated capacity (or >7) is REJECTED. If the requested script count needs more slots than
  capacity×(calendar weeks) allows, EXTEND the calendar to more weeks rather than breach the weekly
  cadence. If capacity exceeds 3/week, add a one-line note that consistency beats volume.
- Pull hooks/formats from the s5 hook bank + transplant map; tag which card/transplant each derives from.
  Every source card ID you cite MUST exist in the loaded s4/s5 set AND the mechanic you borrow must match
  that card's actual decode — cross-check, don't invent or misattribute.
- LABEL each slot proof-led or format-led: proof-led = it carries an owned result (from PROOF ON FILE /
  notes / baseline); format-led = no owned proof, it wins on format alone. A REACH/NURTURE slot whose topic
  matches an available PROOF_ASSET MUST be proof-led — do not write it proof-less to duck to an easier bar.
CALENDAR TABLE — one row per slot: date | week | REACH/NURTURE/CONVERT | proof-led/format-led | pillar |
format (from bank) | hook concept | topic | audio rec | CTA | production-effort tag (phone-only / designer /
on-site shoot — from {{PRODUCTION_MODES}}) | source (card ID or transplant). Print the LITERAL integer counts +
...
- OVERLAY ≠ VERBAL: the overlay must be a genuinely SECOND message, not the spoken hook restated — if it
  is identical OR a near-duplicate reword (same meaning, a word or two swapped), rewrite it.
...
- HOW WE RESEARCHED IT: the funnel counts (from s2 — candidates → kills → survivors), how many accounts
  ...
  naming the tool used per layer (VERBAL / VISUAL / OVERLAY, matching the S3 layer names, from s4). Make the
  rigor visible.
...
- Keep claims evidence-bound; for each headline number cite which vault file it came from. A count that
  was flagged LOW-CONFIDENCE / UNAVAILABLE / PARTIAL / GEO-SKEWED / LOW-FRESHNESS / MARKET-MIX SHORTFALL /
  NEEDS-PROOF / BUDGET-CAPPED upstream must NOT appear as a clean stat here — carry the flag through to the
  client report.
...
- MCP — apify/instagram-reel-scraper on the client's OWN last-30-days:
  { "username": ["{{CLIENT_HANDLE}}"], "resultsLimit": 30, "skipPinnedPosts": true, "onlyPostsNewerThan": "30 days" }
...
- BOUNDED LOOPS: at most {{MAX_REGEN_LOOPS}} repair rounds per gate and ≤3 maker calls per gate total; keep
  ...
  (5 gates × 3 makers) + (5 checkers) + (ceil({{SCRIPTS_TO_WRITE}} ÷ 5) × ({{MAX_REGEN_LOOPS}} + 1) grader
  calls — the grader runs in ≤5-script batches, so N>5 scripts means ceil(N/5) calls per round) +
  deliverables, STOP and report. The grader loop's S4-maker re-invocations count against the same
  ≤3-maker-calls-per-gate budget for Gate 4.
...
- AUTONOMOUS — NEVER WAIT FOR A HUMAN: any maker instruction to "ask the operator", "PAUSE and ask", or
  seek a "proceed" go-ahead (the S2 MINIMUM-POOL gate, the S2/S4 cost guardrails) AUTO-RESOLVES in this
  mode to "proceed LOW-CONFIDENCE", stamped in the FINAL REPORT — the run never blocks on human input; it
  only stops on a HARD-STOP condition above.
...
GATE 2 — Discover & Rank (maker S2, Sonnet) — SAME SESSION as Gate 3 (IG video URLs expire). Checker
  ...
  90d, selected reels inside the 30–45d selection window — out-of-window reels are allowed ONLY if tagged
  "evergreen-format reference" or the run carries a LOW-FRESHNESS flag; freshness receipt + pool date-range
  present); SHORTFALLs flagged not padded; origins recorded; ORIGIN MIX line present (MARKET MIX ≥5 met or
  SHORTFALL printed — never padded); s2-discovery.md + s3-outliers.md saved (with per-account datasetIds). Cap
  deep-scrape to ceil({{REELS_TO_DECODE}} ÷ 2) accounts; enforce the budget governor. SPOT-CHECK (the outlier
  SCORE is the single most important number — verify the WHOLE ratio, not just plays): for 3 random ranked
  accounts, fetch their recorded datasetId items (read-only, no new scrape), RECOMPUTE the median, and assert
  both the reel's plays AND OUTLIER_SCORE = plays ÷ recomputed median (±10%) match the table — any mismatch =
  FAIL (fabricated numbers or a doctored divisor).
GATE 3 — Decode & Synthesize (maker S3, Sonnet). Checker (Haiku): VERBAL states detected-language + engine
  ...
  THE GAP cites ≥3 card IDs; both files saved. DECODE SPOT-CHECK (mirror the Gate-2 pattern on the decode
  layer — the most fabrication-prone one): s4-decode must record each reel's decode-actor runId/datasetId;
  the checker re-fetches ≤3 transcript/visual dataset items and diffs them VERBATIM against the card — a card
  whose VERBAL/OVERLAY doesn't match the actor output = FAIL. PLUS an ANTI-FABRICATION check (Opus — the only
  Opus checker): does any VERBAL read as written-from-caption rather than a real transcript? A non-English reel
...
GATE 4 — Plan & Script (maker S4, OPUS — the one creative stage) → then the GRADER loop (below).
  ...
  (Haiku): COVERAGE X==N (X<N = FAIL); cadence matches {{POSTING_CAPACITY}} and ≤7/wk; slots ≤
  {{POSTING_CAPACITY}}×(calendar weeks) — if {{SCRIPTS_TO_WRITE}} exceeds that, the calendar SPANS more weeks
  rather than breaching cadence; 40/40/20 reported with integer counts AND ≥1 CONVERT slot present (a
  0-CONVERT calendar = FAIL — CONVERT is the revenue bucket; stamp it "needs proof" when none on file, never
  drop it); every cited card ID EXISTS in s4-decode AND its cited mechanic MATCHES that card (a real C#
  with a misattributed mechanic = FAIL); each script's first row carries all 3 hook channels with
  OVERLAY≠VERBAL; each script has the FOOTER (HOOK / CTA / WHY-VIRAL), beat timecodes that sum to the target
  length, and a caption + hashtags (a one-line "hook only" script = FAIL); scripts are written in the
  client's language (wrong-language output = FAIL); CONVERT slots cite real proof or are flagged "needs
  proof" (never invented); NO-GO clean; s6-calendar.md + s7-scripts.md saved. RECORD each script's bucket
  (REACH/NURTURE/CONVERT) and proof-led vs format-led status in s6-calendar.md — a REACH/NURTURE slot whose
  topic matches an available PROOF_ASSET MUST be recorded proof-led (MIN bar); "format-led" is valid ONLY
  when no relevant proof exists (writing a slot proof-less to duck to the MIN−3 bar = FAIL). The grader MUST
  use these recorded labels; a label that changes between grading rounds = FAIL (relabeling to reach a lower
  bar is gaming).
GATE 5 — Showcase + Learning Loop (maker S5, Sonnet). Checker (Haiku): 7-file LOAD RECEIPT complete with
  ...
  BLOCKED-stamped script counts as covered but must display its stamp). FLAG + NUMBER DIFF (mechanical, not
  eyeball — the absence of a flag is invisible otherwise): the checker LOADS s3-outliers + s4-decode +
  s5-patterns + s6-calendar, ENUMERATES every quality/degrade flag (LOW-CONFIDENCE / UNAVAILABLE / PARTIAL /
  GEO-SKEWED / LOW-FRESHNESS / MARKET-MIX SHORTFALL / NEEDS-PROOF / BUDGET-CAPPED) and every headline number,
  and asserts each flag appears in the report and each cited number matches the file it cites — a laundered
  flag (dropped) or a number with no matching source = FAIL; s8-report.md + s8b-learning-loop.md saved.
...
THE VIRALITY GRADER (Gate 4, Sonnet — ONE grader; Higgsfield virality_predictor is NOT used, it needs a
finished video). The grader scores the script TEXT but MUST LOAD s4-decode + s5-patterns and VERIFY every
cited C#: the card must exist AND its actual decoded mechanic must match what the script claims to borrow —
a script self-asserting a C# it doesn't match, or citing a mechanic that card doesn't have, earns NO credit
on the axis that leaned on it (this is what stops the "just name any plausible C#" game). Grade every script
0–100. Score each axis 0–10 with a quoted justification that cites the SPECIFIC script line AND the verified
card's mechanic; a bare assertion with no quoted line caps that axis at 3. Axes and weights (they sum to 100):
  Hook strength (verbal) 18 · Visual hook 14 (must name the decoded C# whose first-frame mechanic it
  borrows → else cap 5; an invented visual hook is not evidence) · Overlay hook 12 (identical to OR a
  near-paraphrase of the spoken hook → cap 3) ·
  ...
  placeholder marked" is graded with the owned-proof axis EXCLUDED (re-weight the remaining 88 points
  PROPORTIONALLY — each surviving axis_weight × 100/88 — so the axes still sum to 100; state the reweight)
  and delivered carrying a NEEDS-PROOF stamp — a missing client asset is not a script defect and
...
FAITHFULNESS GATE (before emitting) — two directions, like the Beautifier's no-silent-drop rule:
  (1) FRAME→SOURCE: every frame's VO + OVERLAY is a verbatim line from a source script row. ALSO: any text
      the IMAGE-GEN PROMPT bakes into the frame must be a verbatim script overlay, and any RESULT/claim the
      prompt depicts (a "−12 lbs", a "before/after", a number on screen) must trace to a script line — a
      frame may NOT invent an on-screen result, caption, or claim the script doesn't contain (this is the
      most dangerous fabrication in a client deliverable).
  ...
  BLIND CHECK: run this faithfulness gate as a SEPARATE blind sub-agent (fresh eyes — given the frames +
  the source script, NOT the maker's reasoning), consistent with the pipeline's maker→checker rule.
